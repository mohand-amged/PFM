...
import { ArrowLeft, Settings, AlertTriangle, DollarSign, Target, Coins, CheckCircle, XCircle } from 'lucide-react';
...
export default function WalletSettingsForm({ wallet }: WalletSettingsFormProps) {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      if (result.success) {
        setMessage({ type: 'success', text: 'Wallet settings updated successfully! Currency changed to ' + formData.get('currency') });
        // Refresh the page to show updated currency
        setTimeout(() => {
          window.location.reload();
        }, 1500);
      } else {
        setMessage({ type: 'error', text: result.error || 'Failed to update wallet settings. Please try again.' });
      }
    } catch (error) {
      console.error('Error updating wallet:', error);
      setMessage({ type: 'error', text: 'Failed to update wallet settings. Please try again.' });
    } finally {
